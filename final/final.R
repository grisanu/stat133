...
name <- "Grisanu Naing"
github.name <- "matt.grisanu"
email.address <- "matt.jgrisanu@berkeley.edu"
...
# a random permutation of the even numbers from 2 to 222
  set.seed(31415)
v <- c(sample(c(2,0),1), sample(seq(0,8,2),1), sample(c(0,2),1))
...
set.seed(71)
l <- lapply(1:12, function(y) rpois(100,5))
...
f1 = family[family$gender == "f",]

  
# [1 pt]
# Create [f2] a subset of family with only men 6 foot tall or more
f2 <- family[family$gender == "m",]
f2 = family[family$height >= 72,]
...
t <- infants[(infants$marital == "Married"),]
t <- infants[(infants$parity == 1),]
...
boxplot(iris[iris$Species == "setosa",]$Sepal.Length, ylim = c(3,9))
boxplot(iris[iris$Species == "versicolor",]$Sepal.Length, add = T)
boxplot(iris[iris$Species == "virginica",]$Sepal.Length, add = T)
...
plot(iris$Petal.Length, iris$Petal.Width, col = col, xlab = "Petal Length", ylab = "Petal Width")

# [3 pt]
# Make a scatterplot of ( sepal length / petal length) as a function of index (order)
# Color the plotting symbol by Species (any 3 colors)
temp = (iris$Sepal.Length)/ (iris$Petal.Length);

plot(1:dim(iris)[1], temp, col =col);
...
plot(infants$gestation, infants$bwt, col = "Red", xlab = "infant gestation time in days", ylab = "infant birthweight in ounces", pc = "*") ######
abline(v = 259)
...
plot(density(infants$age, na.rm =T), col = "blue", ylim = c(0, 200))
hist(infants$age, col = "red", xlab = "infant mother's age", add = T)
#################################################################
...
first.cache <- sapply(1:length(Cache500), function(x) Cache500[[x]][1])

# [3 pts]
# Create [mean.cache], a vector of length 500 where each entry is the mean 
# of the corresponding element of the list Cache500

mean.cache <- sapply(1:length(Cache500), function(x) mean(Cache500[[x]], na.rm = T))

# [2 pts]
# Create [sd.cache], a vector of length 500 where each entry is the sd
# of the corresponding element of the list Cache500

sd.cache <- sapply(1:length(Cache500), function(x) sd(Cache500[[x]], na.rm = T))
...
mean.long.cache = sapply(1:length(Cache500), function(x) if (length(Cache500[[x]]) >= 50) {
  mean(Cache500[[x]], na.rm = T)  
}else{NA})
...
temp_set = max(iris[iris$Species == "setosa",]$Petal.Length, na.rm = T)
temp_ver = max(iris[iris$Species == "versicolor",]$Petal.Length, na.rm = T)
temp_vir = max(iris[iris$Species == "virginica",]$Petal.Length, na.rm = T)

max.petal.width <- c(setosa = temp_set, versicolor = temp_ver, virginica = temp_vir)
...
firstColToNames <- function(mat){
  trunc_col = mat[,2:dim(mat)[2]];
  row.names(trunc_col) = mat[,1];
  return(trunc_col)
}
...
longerRange <- function(m1, m2){
  ran_m1 = max(apply(m1, 2, max, na.rm = T), na.rm = T) - min(apply(m1, 2, min, na.rm = T), na.rm = T);
  
  ran_m2 = max(apply(m2, 2, max, na.rm = T), na.rm = T) - min(apply(m2, 2, min, na.rm = T), na.rm = T);
...
dice_sum <- function(k, B){
  dsum = sapply(1:B, function(x) sum(sample(1:6, k, replace = T)))
  return(dsum)
}
...
# For each value of B we will:
  ave.diceRoll <- c(rep(0,4))
  sd.diceRoll <- c(rep(0,4))
...
for (i in 1:4) {
  output1 = dice_sum(k_q, Bvec[i])
  ave.diceRoll[i] = mean(output1)
  sd.diceRoll[i] = sd(output1)
  hist(output1, xlab = "sum of dice roll", main = c("Histogram for B =", Bvec[i]))
}
...
phrases_temp = gsub("[[:punct:]]", "u", phrases)
text2 <- grep("(at)\\>", phrases_temp)
...
tests <- sapply(1:200, function(x) paste("test", x, sep = ""))
...
# minchin.split = gregexpr("[A-Z]", minchin)
# minchin.split <- unlist(strsplit(minchin, " "))

minchin.split = gsub("A", "a", minchin);
minchin.split = gsub("B", "b", minchin.split);
minchin.split = gsub("I", "i", minchin.split);
minchin.split = gsub("T", "t", minchin.split);
minchin.split = gsub("M", "m", minchin.split);
minchin.split = gsub("H", "h", minchin.split);
minchin.split = gsub("Y", "y", minchin.split);
minchin.split = gsub("O", "o", minchin.split);

minchin.split <- unlist(strsplit(minchin.split, " "))
#################################################################
